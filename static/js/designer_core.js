console.log("designer_core loaded");
window.ReelveDesigner = (() => {
  function init({ rootId, prefix = "" }) {
    ...
    const canvas = q("design-canvas");
    const drawLayer = q("draw-layer");
    const toolButtons = qa(`.${prefix ? "ed-tool-btn" : "tool-btn"}`);

    const imageUpload = q("image-upload");
    const bgColor = q("bg-color");
    const fontSize = q("font-size");
    const fontFamily = q("font-family");
    const textColor = q("text-color");
    const letterSpacing = q("letter-spacing");
    const lineHeight = q("line-height");
    const alignLeft = q("align-left");
    const alignCenter = q("align-center");
    const alignRight = q("align-right");
    const textBold = q("text-bold");
    const textItalic = q("text-italic");
    const textUpper = q("text-upper");
    const fillColor = q("fill-color");
    const strokeColor = q("stroke-color");
    const strokeWidth = q("stroke-width");
    const brushSize = q("brush-size");
    const drawOnSelection = q("draw-on-selection");
    const bringFront = q("bring-front");
    const sendBack = q("send-back");
    const shapeType = q("shape-type");
    const deleteLayerBtn = q("delete-layer");
    const exportBtn = q("export-post") || document.getElementById(`${prefix}export-post`);
    const exportTop = q("export-post-top") || document.getElementById(`${prefix}export-post-top`);
    const rotateInput = q("rotate");
    const canvasPreset = q("canvas-preset");
    const canvasWidth = q("canvas-width");
    const canvasHeight = q("canvas-height");
    const applyCanvas = q("apply-canvas");
    ...
    async function handleExport(e) {
      ...
      const canvasEl = root.querySelector(`#${prefix}design-canvas`) || document.getElementById(`${prefix}design-canvas`);
      if (!canvasEl) {
